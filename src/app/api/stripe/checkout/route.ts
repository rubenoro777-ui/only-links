...
export async function POST(request: NextRequest) {
  ...
  if (body.type === "link_unlock") {
    ...
    const { data: linkRowRaw } = await supabase
      .from("links")
      .select(
        "id, title, price_cents, profiles(handle, stripe_connect_account_id, stripe_connect_charges_enabled, subscription_status)",
      )
      .eq("id", linkId)
      .eq("is_locked", true)
      .maybeSingle();
    ...
    const tier = isOwnerAccount({ handle: ownerProfile.handle })
      ? "pro"
      : subscriptionTierFromStatus(ownerProfile.subscription_status);
